...
def solve() :
    ...
    multitap = stuff[:n]
    change(n,m,stuff,multitap)
    
def change(n,m,stuff,multitap) :
    result = 0
    for i in range(n,m) :
        if stuff[i] in multitap :
            continue
        
        stuffList = []
        for ii in range(n) :
            try :
                stuffNumber = stuff[i:].index(multitap[ii])
            except :
                stuffNumber = 101
            stuffList.append(stuffNumber)
            
        outStuff = stuffList.index(max(stuffList))
        del multitap[outStuff]
        multitap.append(stuff[i])
        result += 1
